...
train_image_file_names = [TRAIN_DIR+i for i in os.listdir(TRAIN_DIR)][0:10000] 
test_image_file_names = [TEST_DIR+i for i in os.listdir(TEST_DIR)][0:447]
...
def compute_accuracy(v_xs, v_ys):
    global prediction
    y_pre = sess.run(prediction, feed_dict={xs_before_reshape: v_xs})
    correct_prediction = tf.equal(tf.argmax(y_pre,1), tf.argmax(v_ys,1))
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
    result = sess.run(accuracy, feed_dict={xs_before_reshape: v_xs, ys: v_ys})
    return result

# define placeholder for inputs to network
xs_before_reshape = tf.placeholder(tf.float32, shape=[None, 28 , 28 , 3]) # 
xs = tf.reshape(xs_before_reshape, shape=[-1, 28 * 28 * 3])
ys = tf.placeholder(tf.float32, shape=[None, 2])

# add output layer
l1 = add_layer(xs, 28*28*3, 373, 'l1', activation_function=tf.nn.tanh)
prediction = add_layer(l1,373, 2, 'l2', activation_function=tf.nn.relu)
# prediction = add_layer(l1,373, 2, 'l2', activation_function=tf.nn.softmax)
...
for i in range(1000):
    ...
    sess.run(train_step, feed_dict={xs_before_reshape: processed_train_images, ys: labels})
    # y_pre = sess.run(prediction, feed_dict={xs_before_reshape: processed_train_images})
    if i % 50 == 0:
